...
When J<sub>pred</sub>=0, structural checks already force the third branch, so M<sub>A</sub>, M<sub>B</sub>, D<sub>tr</sub>, and terminal effect agreement are not evaluated. A wire-format zero in those fields is a placeholder accompanied by a false `prediction-evaluated` flag, not a measured zero.
...
For bounded chain length, the local execution cost is T<sub>chain</sub>=T<sub>struct</sub>+J<sub>pred</sub>T<sub>law</sub>. Structural validation and a warm exact-score hit are each O(m<sub>ch</sub>); a cold dense-vector audit is O(m<sub>ch</sub>d) for embedding dimension d. Store access, remote embedding, cache maintenance, and concurrency are measured separately.
...
Claim 4a. A declared cross-domain correspondence is a predictive chain-translation candidate only when the mapped distinction propagates through every aligned stage, all non-target coordinates remain fixed, and the signed predictor-effect defect is bounded. If the upstream distinction is erased at the terminal stage and both domain movements are small, it is instead a higher-order invariant candidate. Any failed intermediate square remains divergence. This observed-path criterion does not establish equality of unknown transition kernels, discover the correspondence, or support a causal label without identified interventions and transport assumptions.
...
M<sub>A</sub>,M<sub>B</sub>,D<sub>tr</sub>,η<sub>inv</sub>,η<sub>tr</sub>,𝒞<sub>chain</sub>: domain-wise predictor movement, cross-domain signed-effect defect, frozen invariant and translation thresholds, and invariant/translation/divergence classifier.
