...
When repetition protection is enabled, the Section 6 descriptor, packing-occupancy, exemption, and selected-feedback rules are also frozen under $\Xi_B$ and $\Lambda_{\mathrm{eval}}$. The packet rule changes retrieval output only; the feedback rule can change subsequent learned state and therefore future scored laws. Its full admission semantics cannot be replaced by a claim of source independence from lexical similarity or a hash.
...
Claim 4a. A declared cross-domain correspondence is a predictive chain-translation candidate only when the mapped distinction propagates through every aligned stage, all non-target coordinates remain fixed, the complete signed predictor-effect defect satisfies $D_{\mathrm{tr}}\le\eta_{\mathrm{tr}}$, and the terminal effect defect satisfies $D_{\mathrm{term}}\le\eta_{\mathrm{tr}}$. If the upstream distinction is erased at the terminal stage and both domain movements satisfy $M_A,M_B\le\eta_{\mathrm{inv}}$, it is instead a higher-order invariant candidate; zero motion is permitted as a vacuous proposal-only label unless a positive floor is preregistered. Any failed intermediate square remains divergence. This observed-path criterion does not establish equality of unknown transition kernels, discover the correspondence, or support a causal label without identified interventions and transport assumptions.
