...
| 2b | The revised monitor meets the frozen v5 synthetic scenario criteria. | Validated in fixture | V6 confirmation passed all six cases: noisy abrupt 87.5%, gradual 95.31%, recurring 79.69%, with 0.2031, 0.1875, and 0.2188 unmatched alarms per trajectory. |
| 2b | Omitted-influence certificates attain the declared empirical coverage. | Inconclusive; estimator absent | The runtime validates externally supplied fields but does not estimate shadow-audit coverage or never-nominated influence. Synthetic asserted certificates are not empirical evidence. |
...
The frozen protocol, generated reports, synthetic dataset, aggregate chronological reports, and machine-readable snap test are preserved under `evidence/claims-completion/`. Raw private session text is not included. The remaining protocols below are required to move beyond this evidence.
...
The eighth open problem is residual confidence under drift. Residuals are statistical corrections, not causal hypotheses. The heterogeneous confirmation improved average Brier loss while harming 41.34% of applied cases, demonstrating that average utility and safe reuse are different requirements. A stronger theory and implementation must specify decay schedules, effective sample size, false-correction costs, changepoint coupling, and a gate that suppresses stale corrections without erasing stable-bias gains.
...
The nineteenth open problem is predictive sheaf-snap search. The current service publishes, invalidates, and rolls back graph versions but does not yet feed the graph into nomination, neighbor compatibility, or forecast composition; its accepted snap therefore left scored output unchanged. Candidate families must be expressive enough to repair local incompatibility but bounded enough to avoid combinatorial search, repeated-test overfitting, and graph churn. Theory and implementation are needed for predictive graph consumption, neighborhood selection, edit penalties, rollback, and cache-hit recovery under drift.
...
The twenty-third open problem is posterior granularity. Anti-Pigeon supplies an external divergence gate for sharing, and the bounded Beta-Bernoulli comparator supplies a conservative proposal mechanism. Independent confirmation identified all strong and 90.63% of moderate splits but left every ordinary compatible noisy group uncertain, failing the frozen complete share-versus-split criterion. Reliable positive sharing evidence, optimal split and merge policies, and external certificate coverage under sparse evidence, multiple horizons, source dependence, and drift remain unknown. Over-sharing creates confident category errors; over-splitting wastes evidence and memory.

The twenty-fourth open problem is bounded changepoint inference. Exact Bayesian online changepoint support grows with stream length. The implemented capped run-length plus two-sided cumulative detector met all six frozen synthetic scenario criteria, but noisy recurring detection remained 79.69%, changing streams still produced unmatched alarms, and gradual mean delay was 45.44 observations. Truncation, pruning, finite-state approximation, cooldown, and cumulative thresholds require error and stability bounds that remain meaningful under selective activation, delayed labels, and repeated changes.

The twenty-fifth open problem is omitted influence. A bounded local frontier can miss weak individual signals whose joint effect is material. The current runtime validates externally issued omitted-influence records but does not implement the shadow-audit estimator needed to demonstrate their coverage, including never-nominated events. Independent non-admitted-event audits would estimate this risk only on sampled candidates; estimator construction and coverage guarantees under adversarial or highly correlated omissions remain open.
