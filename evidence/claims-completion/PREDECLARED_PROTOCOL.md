...
1. **Chronological software-session track.** Read-only sessions with timestamps in
...
The anchor label is a retrieval-usefulness proxy, not causal task success.
Injected drift and known-truth group experiments establish mechanism behavior,
not prevalence in production. If fewer than three post-cutoff trajectories
